...
export function PublicTopNav() {
  ...
  const navItems = [
    { label: t('create'), href: '/ai/image' },
    { label: t('video'), href: '/ai/video' },
    { label: t('presets'), href: '/presets' },
    { label: t('community'), href: '/community' },
    { label: t('pricing'), href: '/pricing' },
  ];

  return (
    <header className="sticky top-0 z-30 border-b border-white/10 bg-black/82 px-4 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4">
        <a href="/" className="flex items-center gap-2 text-sm font-semibold text-white">
          ...
          <span className="tracking-wide">Amux Studio</span>
        </a>
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="rounded-md px-3 py-2 text-sm text-white/70 transition hover:bg-white/8 hover:text-white"
            >
              {item.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-2">
          <a
            href="/login"
            className="hidden rounded-md px-3 py-2 text-sm text-white/70 transition hover:bg-white/8 hover:text-white sm:inline-flex"
          >
            {t('signIn')}
          </a>
          <a
            href="/workbench/image"
            className="inline-flex min-h-10 items-center gap-2 rounded-md bg-white px-4 py-2 text-sm font-semibold text-black transition hover:bg-[#c9ff82]"
          >
