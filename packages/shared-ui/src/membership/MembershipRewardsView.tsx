...
type MembershipRewardsViewProps = {
...
function getErrorMessage(error: unknown, fallback: string) {
  ...
  return (
    (typeof maybeResponse.response?.data?.message === 'string'
      ? maybeResponse.response.data.message
      : null) ??
    (typeof maybeResponse.message === 'string' ? maybeResponse.message : null) ??
    fallback
  );
}
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      >
        ...
        <div className="ml-auto">
          <Button size="sm" variant="ghost" disabled={isRefreshing} onClick={() => void refresh()}>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        {error && (
          ...
          >
            {getErrorMessage(error, t('rewardsLoadFailed'))}
          </div>
        )}
        ...
        <section className="mb-5">
          ...
          {isLoading ? (
          ...
          ) : progressData?.activeCampaigns.length ? (
            <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
              {progressData.activeCampaigns.map((campaign) => (
                <article
                  key={campaign.id}
            ...
            <p
              className="rounded-lg px-4 py-8 text-sm"
        ...
        <section className="mb-5">
          ...
          {progressData?.pendingInvites.length ? (
            <div className="rounded-lg" style={{ border: '1px solid var(--border)' }}>
              {progressData.pendingInvites.map((invite) => (
                <div
                  key={invite.id}
                ...
                >
                  <span style={{ color: 'var(--foreground)' }}>
                    {t('invitedUser', { userId: invite.inviteeUserId })}
                  </span>
        ...
        <section>
          ...
          {progressData?.rewards.length ? (
            <div className="overflow-hidden rounded-lg" style={{ border: '1px solid var(--border)' }}>
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ backgroundColor: 'var(--surface-secondary)' }}>
                    <th className="px-4 py-2.5 text-left text-xs font-medium" style={{ color: 'var(--muted)' }}>
                      {t('campaign')}
                    </th>
                    <th className="px-4 py-2.5 text-right text-xs font-medium" style={{ color: 'var(--muted)' }}>
                      {t('points')}
                    </th>
                    <th className="px-4 py-2.5 text-left text-xs font-medium" style={{ color: 'var(--muted)' }}>
                      {t('pointsRecordTime')}
                ...
                <tbody>
                  {progressData.rewards.map((reward) => (
