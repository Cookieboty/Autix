...
export function AdminPermissionCenterView() {
  ...
  const handleDeletePermission = (permissionId: string) => {
    setDeleteConfirm({
      type: 'permission',
      id: permissionId,
      name: t('thisPermission'),
    });
  };

  const handleSystemSubmit = async (data: AdminSystemFormInput) => {
  ...
  const handleMenuSubmit = async (data: AdminMenuFormInput) => {
    if (editingItem && 'id' in editingItem) {
  ...
  const handlePermissionSubmit = async (data: AdminPermissionFormInput) => {
    if (editingItem && 'id' in editingItem) {
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (deleteConfirm.type === 'system') {
                      deleteSystemMutation.mutate(deleteConfirm.id);
                    }
                    if (deleteConfirm.type === 'menu') {
                      deleteMenuMutation.mutate(deleteConfirm.id);
                    }
                    if (deleteConfirm.type === 'permission') {
                      deletePermissionMutation.mutate(deleteConfirm.id);
                    }
