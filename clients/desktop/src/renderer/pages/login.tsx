...
export function LoginPage() {
  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      setError(e.msg || e.response?.data?.msg || t('loginFailed'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-background">
        <div className="w-full max-w-md space-y-8">
            ...
            {error && (
              <div
                ...
                style={{
                  ...
                  border:
                    '1px solid color-mix(in oklch, var(--danger) 20%, transparent)',
                }}
                role="alert"
              >
                {error}
          ...
          <p className="text-center text-xs text-foreground/30">{t('copyright')}</p>
        </div>
      </div>
    </div>
  );
