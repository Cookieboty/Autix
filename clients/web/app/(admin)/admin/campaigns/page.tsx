    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[1fr_420px]">
        <div className="min-h-0 overflow-y-auto">
          {loading ? (
            <div className="flex items-center justify-center py-20 text-sm" style={{ color: 'var(--muted)' }}>{tCommon('loading')}</div>
          ) : campaigns.length === 0 ? (
            <div className="flex items-center justify-center py-20 text-sm" style={{ color: 'var(--muted)' }}>{t('empty')}</div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  <th className="px-4 py-3 text-left text-xs font-medium" style={{ color: 'var(--muted)' }}>{t('campaign')}</th>
                  <th className="px-4 py-3 text-left text-xs font-medium" style={{ color: 'var(--muted)' }}>{t('typeLabel')}</th>
                  <th className="px-4 py-3 text-right text-xs font-medium" style={{ color: 'var(--muted)' }}>{t('reward')}</th>
                  <th className="px-4 py-3 text-right text-xs font-medium" style={{ color: 'var(--muted)' }}>{t('budget')}</th>
                  <th className="px-4 py-3 text-left text-xs font-medium" style={{ color: 'var(--muted)' }}>{t('statusLabel')}</th>
                  <th className="px-4 py-3 text-right text-xs font-medium" style={{ color: 'var(--muted)' }}>{t('operations')}</th>
                </tr>
              </thead>
              <tbody>
                {campaigns.map((campaign) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3" style={{ color: 'var(--foreground)' }}>{campaignTypeLabel(campaign.type)}</td>
          ...
          {selected ? (
            <>
              ...
              <div className="mb-4 rounded-lg p-4" style={{ border: '1px solid var(--border)' }}>
                <h3 className="mb-2 text-sm font-semibold" style={{ color: 'var(--foreground)' }}>{t('manualGrant')}</h3>
              ...
              <h3 className="mb-2 text-sm font-semibold" style={{ color: 'var(--foreground)' }}>{t('recentRewards')}</h3>
              {rewards.length ? (
                ...
                <p className="rounded-lg p-4 text-sm" style={{ color: 'var(--muted)', border: '1px solid var(--border)' }}>{t('noRewards')}</p>
              )}
            </>
          ) : (
            <p className="text-sm" style={{ color: 'var(--muted)' }}>{t('selectHint')}</p>
          )}
        </aside>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          >
            ...
            <div className="mt-5 flex justify-end gap-2">
              <Button size="sm" variant="ghost" onClick={() => setModal(null)}>{tCommon('cancel')}</Button>
