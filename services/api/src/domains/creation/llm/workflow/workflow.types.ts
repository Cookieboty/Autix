...
export type WorkflowGeneratedImageRef = WorkflowImageRef & {
  sourceImages?: WorkflowImageRef[];
  referenceImages?: WorkflowImageRef[];
};
...
export type WorkflowStepEvent =
  ...
  | { type: 'edit_suggestion'; instruction: string; sourceImages: WorkflowImageRef[]; model: string; reasoning: string }
  | { type: 'image_generating'; taskId: string; model: string; count: number }
  | { type: 'image_editing'; taskId: string; model: string; sourceImages: WorkflowImageRef[]; count: number }
  | { type: 'image_generated'; taskId: string; images: Array<string | WorkflowGeneratedImageRef>; prompt: string; model: string; sourceImages?: WorkflowImageRef[]; referenceImages?: WorkflowImageRef[] }
