...
// ── batchModerate（Task 3）──────────────────────────────────────────────

describe('GalleryService.batchModerate — 逐条尽力执行', () => {
  const adminId = 'admin-1';

  /** 审核动作专用工厂：需要 findById / update / writeAuditLog。 */
  function makeAdminService(posts: Record<string, { status: string }>) {
    const auditLogs: { action: string; targetId: string }[] = [];
    const updated: string[] = [];
    const repo = {
      findById: async (id: string) => (posts[id] ? { id, ...posts[id] } : null),
      update: async (id: string, data: Record<string, unknown>) => {
        updated.push(id);
        return { id, ...posts[id], ...data };
      },
      removeAndArchiveTemplate: async (id: string) => {
        updated.push(id);
        return { id };
      },
      writeAuditLog: async (action: string, _actorId: string, payload: Record<string, unknown>) => {
        auditLogs.push({ action, targetId: payload.targetId as string });
        return { id: 'log-1' };
      },
    };
    const service = new GalleryService(repo as never, {} as never, {} as never);
    return { service, auditLogs, updated };
  }

  it('部分失败：状态非法的条目进 failed，其余照常进 succeeded', async () => {
    const { service, auditLogs } = makeAdminService({
      'p-1': { status: GalleryStatus.PENDING },
      'p-2': { status: GalleryStatus.PUBLISHED }, // 已被别人通过 → PUBLISHED→PUBLISHED 非法
      'p-3': { status: GalleryStatus.PENDING },
    });

    const result = await service.batchModerate(adminId, {
      ids: ['p-1', 'p-2', 'p-3'],
      action: 'approve',
    } as never);
  ...
  it('不存在的 id 进 failed，不影响其余条目', async () => {
    const { service } = makeAdminService({ 'p-1': { status: GalleryStatus.PENDING } });
  ...
  it('reject 把共用 reason 透传给每一条', async () => {
    ...
    const { service } = makeAdminService({
      'p-1': { status: GalleryStatus.PENDING },
      'p-2': { status: GalleryStatus.PENDING },
    });
    const spy = vi.spyOn(service, 'reject');

    await service.batchModerate(adminId, {
      ids: ['p-1', 'p-2'],
      action: 'reject',
      reason: '不符合社区规范',
    } as never);

    spy.mock.calls.forEach((call) => captured.push((call[2] as { reason: string }).reason));
    expect(captured).toEqual(['不符合社区规范', '不符合社区规范']);
  });

  it('全部成功时 failed 为空数组', async () => {
    const { service } = makeAdminService({
      'p-1': { status: GalleryStatus.PENDING },
      'p-2': { status: GalleryStatus.PENDING },
    });
