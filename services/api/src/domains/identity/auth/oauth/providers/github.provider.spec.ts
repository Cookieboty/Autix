...
function makeProvider(over: any = {}) {
  return new GitHubProvider(
    { clientId: 'cid', clientSecret: 'sec', redirectUri: 'http://localhost:3100/api/auth/callback/github' },
...
describe('GitHubProvider', () => {
  it('buildAuthorizeUrl 含 scope、state 与 PKCE(S256)', () => {
    const url = new URL(makeProvider().buildAuthorizeUrl({ state: 'st', codeChallenge: 'cc' }));
  ...
  it('exchangeCode 透传 code_verifier 给 token 交换', async () => {
    ...
    expect(exchange).toHaveBeenCalledWith('c', 'v1');
  });
