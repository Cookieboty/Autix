...
const LIST_FORBIDDEN_KEYS = ['prompt', 'paramsSnapshot', 'upstreamBody', 'upstreamDiagnostics'];
...
export class GenerationTaskAdminService {
  ...
  async getDetail(id: string) {
    ...
    // 而不是吞掉这个信号。
    const task2 = task as { billingStatus?: string };
    const dataInconsistent = task2.billingStatus === 'CONFIRMED' && !hold;
